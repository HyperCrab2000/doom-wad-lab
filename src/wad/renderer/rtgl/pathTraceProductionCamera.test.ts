...
function countNonSky(pixels: Uint8Array): number {
  let n = 0;
  for (let i = 0; i < pixels.length; i += 4) {
    if (pixels[i] !== SKY[0] || pixels[i + 1] !== SKY[1] || pixels[i + 2] !== SKY[2]) n++;
  }
  return n;
}

describe('pathTraceProductionCamera', () => {
  it('E1M1 spawn hits geometry using shared playfield camera matrices', () => {
    const map = loadE1M1();
    ...
    updatePlayfieldCamera(playfield, 1280, 900, 45, 0.1, 64000, viewMatrix, modelMatrix);
    ...
    expect(countNonSky(pixels)).toBeGreaterThan(160 * 84 * 0.15);
  });
});
