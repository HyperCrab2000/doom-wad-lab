...
function getvis(x: number, y: number, z: number, xsiz: number, ysiz: number, zsiz: number): number {
...
export function parseKvxSlab6Full(buffer: ArrayBuffer): KvxModel {
  ...
  let ptr = 0;
  function readUint32() {
    let val = dv.getUint32(ptr, true);
    ptr += 4;
    return val;
  }
  function readInt32() {
    let val = dv.getInt32(ptr, true);
    ptr += 4;
    return val;
  }
  function readByte() {
    let val = dv.getUint8(ptr);
    ptr++;
    return val;
  }
