...
const procesSideDef = (
  ...
  otherSideDef: number,
  lineDef: LineDef,
  linkedSkySectors: LinkedSkySectors,
  inverse?: boolean
): Array<SkyBuffer> => {
  ...
  if (skyFlats.indexOf(sector.ceilingpic) >= 0) {
    ...
    if (
      linkedSkySector.height !== sector.ceilingheight &&
      (!otherSector ||
        skyFlats.indexOf(otherSector.ceilingpic) < 0 ||
        linkedSkySector.sectorIndex !== linkedSkySectors.ceilings[otherSide!.sector].sectorIndex)
    ) {
      skys.push(
        createSkyWall(gl, v1, v2, sector.ceilingheight, linkedSkySector.height, Boolean(inverse))
      );
    }
  }
  ...
  if (skyFlats.indexOf(sector.floorpic) >= 0) {
    ...
    if (
      linkedFloorSector.height !== sector.floorheight &&
      (!otherSector ||
        skyFlats.indexOf(otherSector.floorpic) < 0 ||
        linkedFloorSector.sectorIndex !== linkedSkySectors.floors[otherSide!.sector].sectorIndex)
    ) {
