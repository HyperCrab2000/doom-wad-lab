...
const strandsToSectors = (strands: Strands): Sectors => {
  ...
  remainingStrands
    .slice()
    .reverse()
    .forEach((strand, strandIndex, strandObj) => {
...
const completeStrands = (
  remainingStrands: Strands,
  strandsByV2: Record<number, Array<Strand>>
): Sectors | undefined => {
  const sectors: Sectors = [];
...
const deduplicateLines = (lines: Array<Line>): Array<Line> => {
  return (lines = objectValues(
    lines.reduce<Record<string, Line>>((acc, line) => {
...
export const sectorLinesToTriangles = (map: WadMap, lines: Array<Line>): Array<Triangle> => {
  ...
  const strands = groupsLinesIntoStrands(deduplicatedLines, sharedVertices).map((strand) =>
    mergeColinearStrandLines(map, strand)
  );
