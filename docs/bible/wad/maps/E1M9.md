# Map deep dive — E1M9 (Military Base)
...
**E1M9** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M9` |
| Official name | Military Base |
| Notes | Secret level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M9` |
...
Use the Layers panel or programmatic presets on **E1M9**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M9
```
...
```
/?renderer=classic&map=E1M9
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m9-all.png`
