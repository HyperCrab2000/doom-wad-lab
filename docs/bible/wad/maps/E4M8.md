# Map deep dive — E4M8 (Unto the Cruel)
...
**E4M8** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M8` |
| Official name | Unto the Cruel |
| Notes | Near-final |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M8` |
...
Use the Layers panel or programmatic presets on **E4M8**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M8
```
...
```
/?renderer=classic&map=E4M8
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m8-all.png`
