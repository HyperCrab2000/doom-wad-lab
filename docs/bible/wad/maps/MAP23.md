...
**MAP23** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP23` |
| Official name | Map 23 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP23` |
...
Use the Layers panel or programmatic presets on **MAP23**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP23
```
...
```
/?renderer=classic&map=MAP23
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map23-all.png`
