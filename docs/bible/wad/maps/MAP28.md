...
**MAP28** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP28` |
| Official name | Map 28 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP28` |
...
Use the Layers panel or programmatic presets on **MAP28**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP28
```
...
```
/?renderer=classic&map=MAP28
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map28-all.png`
