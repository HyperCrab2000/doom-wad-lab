# Map deep dive — E4M5 (They Will Repent)
...
**E4M5** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M5` |
| Official name | They Will Repent |
| Notes | Crushers + platforms |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M5` |
...
Use the Layers panel or programmatic presets on **E4M5**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M5
```
...
```
/?renderer=classic&map=E4M5
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m5-all.png`
