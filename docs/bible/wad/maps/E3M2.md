# Map deep dive — E3M2 (Slough of Despair)
...
**E3M2** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E3M2` |
| Official name | Slough of Despair |
| Notes | Lava flats |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M2` |
...
Use the Layers panel or programmatic presets on **E3M2**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M2
```
...
```
/?renderer=classic&map=E3M2
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m2-all.png`
