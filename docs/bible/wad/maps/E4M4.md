# Map deep dive — E4M4 (Unruly Evil)
...
**E4M4** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M4` |
| Official name | Unruly Evil |
| Notes | Arch-vile introduction map |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M4` |
...
Use the Layers panel or programmatic presets on **E4M4**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M4
```
...
```
/?renderer=classic&map=E4M4
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m4-all.png`
