...
**E1M1** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M1` |
| Official name | Hangar |
| Notes | Introductory techbase; good layer-test map |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M1` |
...
Use the Layers panel or programmatic presets on **E1M1**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M1
```
...
```
/?renderer=classic&map=E1M1
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m1-all.png`
