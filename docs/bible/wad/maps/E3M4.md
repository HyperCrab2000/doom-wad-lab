# Map deep dive — E3M4 (House of Pain)
...
**E3M4** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E3M4` |
| Official name | House of Pain |
| Notes | Tight BSP |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M4` |
...
Use the Layers panel or programmatic presets on **E3M4**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M4
```
...
```
/?renderer=classic&map=E3M4
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m4-all.png`
