# Map deep dive — E3M3 (Pandemonium)
...
**E3M3** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E3M3` |
| Official name | Pandemonium |
| Notes | Red sky sectors |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M3` |
...
Use the Layers panel or programmatic presets on **E3M3**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M3
```
...
```
/?renderer=classic&map=E3M3
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m3-all.png`
