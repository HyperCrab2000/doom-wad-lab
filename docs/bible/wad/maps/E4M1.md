# Map deep dive — E4M1 (Hell Beneath)
...
**E4M1** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M1` |
| Official name | Hell Beneath |
| Notes | Thy Flesh Consumed start |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M1` |
...
Use the Layers panel or programmatic presets on **E4M1**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M1
```
...
```
/?renderer=classic&map=E4M1
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m1-all.png`
