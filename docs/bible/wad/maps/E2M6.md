# Map deep dive — E2M6 (Halls of the Damned)
...
**E2M6** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E2M6` |
| Official name | Halls of the Damned |
| Notes | Hell transition textures |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E2M6` |
...
Use the Layers panel or programmatic presets on **E2M6**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E2M6
```
...
```
/?renderer=classic&map=E2M6
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e2m6-all.png`
