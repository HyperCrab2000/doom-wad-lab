...
**E3M8** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E3M8` |
| Official name | Dis |
| Notes | Spider Mastermind |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M8` |
...
Use the Layers panel or programmatic presets on **E3M8**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M8
```
...
```
/?renderer=classic&map=E3M8
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m8-all.png`
