# Map deep dive — E1M8 (Phobos Anomaly)
...
**E1M8** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M8` |
| Official name | Phobos Anomaly |
| Notes | Boss gate; special sectors |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M8` |
...
Use the Layers panel or programmatic presets on **E1M8**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M8
```
...
```
/?renderer=classic&map=E1M8
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m8-all.png`
