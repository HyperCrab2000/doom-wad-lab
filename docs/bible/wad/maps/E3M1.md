...
**E3M1** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E3M1` |
| Official name | Hell Keep |
| Notes | Episode 3 hell theme |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M1` |
...
Use the Layers panel or programmatic presets on **E3M1**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M1
```
...
```
/?renderer=classic&map=E3M1
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m1-all.png`
