...
**MAP09** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP09` |
| Official name | Map 09 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP09` |
...
Use the Layers panel or programmatic presets on **MAP09**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP09
```
...
```
/?renderer=classic&map=MAP09
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map09-all.png`
