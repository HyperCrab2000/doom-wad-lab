...
**MAP10** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP10` |
| Official name | Map 10 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP10` |
...
Use the Layers panel or programmatic presets on **MAP10**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP10
```
...
```
/?renderer=classic&map=MAP10
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map10-all.png`
