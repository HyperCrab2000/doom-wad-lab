...
**E2M4** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E2M4` |
| Official name | Deimos Lab |
| Notes | Teleporters |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E2M4` |
...
Use the Layers panel or programmatic presets on **E2M4**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E2M4
```
...
```
/?renderer=classic&map=E2M4
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e2m4-all.png`
