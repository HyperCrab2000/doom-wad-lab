# Map deep dive — E1M6 (Central Processing)
...
**E1M6** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M6` |
| Official name | Central Processing |
| Notes | Large outdoor areas |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M6` |
...
Use the Layers panel or programmatic presets on **E1M6**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M6
```
...
```
/?renderer=classic&map=E1M6
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m6-all.png`
