# Map deep dive — E4M2 (Perfect Hatred)
...
**E4M2** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M2` |
| Official name | Perfect Hatred |
| Notes | Harder combat |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M2` |
...
Use the Layers panel or programmatic presets on **E4M2**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M2
```
...
```
/?renderer=classic&map=E4M2
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m2-all.png`
