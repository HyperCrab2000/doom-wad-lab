...
**MAP03** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP03` |
| Official name | Map 03 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP03` |
...
Use the Layers panel or programmatic presets on **MAP03**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP03
```
...
```
/?renderer=classic&map=MAP03
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map03-all.png`
