...
**MAP32** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP32` |
| Official name | Map 32 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP32` |
...
Use the Layers panel or programmatic presets on **MAP32**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP32
```
...
```
/?renderer=classic&map=MAP32
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map32-all.png`
