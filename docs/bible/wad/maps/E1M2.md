# Map deep dive — E1M2 (Nuclear Plant)
...
**E1M2** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M2` |
| Official name | Nuclear Plant |
| Notes | Slime flats; sector lighting variety |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M2` |
...
Use the Layers panel or programmatic presets on **E1M2**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M2
```
...
```
/?renderer=classic&map=E1M2
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m2-all.png`
