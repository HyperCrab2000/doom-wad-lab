# Map deep dive — E1M7 (Computer Station)
...
**E1M7** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M7` |
| Official name | Computer Station |
| Notes | BSP complexity moderate |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M7` |
...
Use the Layers panel or programmatic presets on **E1M7**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M7
```
...
```
/?renderer=classic&map=E1M7
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m7-all.png`
