# Map deep dive — E3M5 (Unholy Cathedral)
...
**E3M5** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E3M5` |
| Official name | Unholy Cathedral |
| Notes | Tall sectors |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M5` |
...
Use the Layers panel or programmatic presets on **E3M5**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M5
```
...
```
/?renderer=classic&map=E3M5
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m5-all.png`
