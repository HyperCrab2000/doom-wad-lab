...
**MAP13** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP13` |
| Official name | Map 13 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP13` |
...
Use the Layers panel or programmatic presets on **MAP13**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP13
```
...
```
/?renderer=classic&map=MAP13
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map13-all.png`
