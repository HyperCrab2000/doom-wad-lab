# Map deep dive — E1M4 (Command Control)
...
**E1M4** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M4` |
| Official name | Command Control |
| Notes | Switch textures; lifts |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M4` |
...
Use the Layers panel or programmatic presets on **E1M4**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M4
```
...
```
/?renderer=classic&map=E1M4
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m4-all.png`
