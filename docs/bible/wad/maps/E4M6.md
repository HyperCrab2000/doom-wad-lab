# Map deep dive — E4M6 (Against Thee Wickedly)
...
**E4M6** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M6` |
| Official name | Against Thee Wickedly |
| Notes | Large hellscape |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M6` |
...
Use the Layers panel or programmatic presets on **E4M6**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M6
```
...
```
/?renderer=classic&map=E4M6
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m6-all.png`
