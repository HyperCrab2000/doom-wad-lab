...
**E3M9** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.
...
| Notes | Secret |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M9` |
...
Use the Layers panel or programmatic presets on **E3M9**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M9
```
...
```
/?renderer=classic&map=E3M9
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m9-all.png`
