...
**MAP01** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP01` |
| Official name | Map 01 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP01` |
...
Use the Layers panel or programmatic presets on **MAP01**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP01
```
...
```
/?renderer=classic&map=MAP01
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map01-all.png`
