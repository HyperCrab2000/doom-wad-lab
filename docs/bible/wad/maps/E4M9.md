...
**E4M9** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.
...
| Official name | Fear |
| Notes | Secret |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M9` |
...
Use the Layers panel or programmatic presets on **E4M9**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M9
```
...
```
/?renderer=classic&map=E4M9
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m9-all.png`
