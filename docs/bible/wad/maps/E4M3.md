# Map deep dive — E4M3 (Sever the Wicked)
...
**E4M3** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M3` |
| Official name | Sever the Wicked |
| Notes | Teleport chains |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M3` |
...
Use the Layers panel or programmatic presets on **E4M3**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M3
```
...
```
/?renderer=classic&map=E4M3
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m3-all.png`
