# Map deep dive — E4M7 (And Hell Followed)
...
**E4M7** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E4M7` |
| Official name | And Hell Followed |
| Notes | Intense monster counts |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E4M7` |
...
Use the Layers panel or programmatic presets on **E4M7**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E4M7
```
...
```
/?renderer=classic&map=E4M7
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e4m7-all.png`
