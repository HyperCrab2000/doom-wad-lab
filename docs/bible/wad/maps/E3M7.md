# Map deep dive — E3M7 (Gate to Limbo)
...
**E3M7** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E3M7` |
| Official name | Gate to Limbo |
| Notes | Puzzle specials |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E3M7` |
...
Use the Layers panel or programmatic presets on **E3M7**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E3M7
```
...
```
/?renderer=classic&map=E3M7
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e3m7-all.png`
