...
**MAP02** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP02` |
| Official name | Map 02 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP02` |
...
Use the Layers panel or programmatic presets on **MAP02**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP02
```
...
```
/?renderer=classic&map=MAP02
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map02-all.png`
