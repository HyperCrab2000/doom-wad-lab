...
**MAP15** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP15` |
| Official name | Map 15 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP15` |
...
Use the Layers panel or programmatic presets on **MAP15**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP15
```
...
```
/?renderer=classic&map=MAP15
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map15-all.png`
