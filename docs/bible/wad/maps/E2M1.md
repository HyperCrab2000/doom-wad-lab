# Map deep dive — E2M1 (Deimos Anomaly)
...
**E2M1** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E2M1` |
| Official name | Deimos Anomaly |
| Notes | Episode 2 start; brown palette |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E2M1` |
...
Use the Layers panel or programmatic presets on **E2M1**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E2M1
```
...
```
/?renderer=classic&map=E2M1
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e2m1-all.png`
