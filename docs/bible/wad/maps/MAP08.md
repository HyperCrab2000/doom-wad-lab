...
**MAP08** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `MAP08` |
| Official name | Map 08 |
| Notes | Doom II stock level |
| Gold snapshot key | `bspGoldenSnapshots.json` → `MAP08` |
...
Use the Layers panel or programmatic presets on **MAP08**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps MAP08
```
...
```
/?renderer=classic&map=MAP08
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/map08-all.png`
