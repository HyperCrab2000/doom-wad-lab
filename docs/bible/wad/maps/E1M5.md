...
**E1M5** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E1M5` |
| Official name | Phobos Lab |
| Notes | Crushers; narrow corridors |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E1M5` |
...
Use the Layers panel or programmatic presets on **E1M5**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E1M5
```
...
```
/?renderer=classic&map=E1M5
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e1m5-all.png`
