# Map deep dive — E2M2 (Containment Area)
...
**E2M2** is part of the **68-map gold corpus**. Spawn-frame `ref.png` from GZDoom GLES is the pixel oracle for this level.

| Field | Value |
|-------|-------|
| Map ID | `E2M2` |
| Official name | Containment Area |
| Notes | Nukage pools |
| Gold snapshot key | `bspGoldenSnapshots.json` → `E2M2` |
...
Use the Layers panel or programmatic presets on **E2M2**:
...
tsx tools/gzrender-v2/gzdoom-wasm-corpus.mts --maps E2M2
```
...
```
/?renderer=classic&map=E2M2
window.__applyClassicLayerPreset('walls-off')
```

Screenshot corpus template: `docs/bible/classic-layers/screenshots/e2m2-all.png`
