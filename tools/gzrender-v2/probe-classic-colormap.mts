#!/usr/bin/env npx tsx
/** Probe Classic frameParity capture: colormap flag + sample pixels vs gold. */
...
const GOLD = 'artifacts/gzrender-v2/gold-standard/DOOM/E1M1/ref.png';
...
async function main(): Promise<void> {
  ...
  await page.evaluateOnNewDocument(() => {
    ...
    HTMLCanvasElement.prototype.getContext = function (id, attrs) {
      if (id === 'webgl2' || id === 'webgl') {
        attrs = { ...(attrs as Record<string, unknown> ?? {}), preserveDrawingBuffer: true };
      }
  ...
  const stats = await page.evaluate(() => {
    const raw = (window as { __doomDrawStats?: Record<string, unknown> }).__doomDrawStats ?? {};
    return {
      gzdoomColormap: raw.gzdoomColormap,
      walls: raw.walls,
      flats: raw.flats,
      wallSkippedTex: raw.wallSkippedTex,
      parityWallGlobVis: raw.parityWallGlobVis,
      flatSectors: raw.flatSectors,
      flatSubsectorSectors: raw.flatSubsectorSectors,
      cameraSectorIndex: raw.cameraSectorIndex,
      skyActive: raw.skyActive,
      outdoorSkyOpening: raw.outdoorSkyOpening,
      ceilingSkySkips: raw.ceilingSkySkips,
      backend: document.querySelector('.level-viewer')?.getAttribute('data-render-backend'),
    };
  });

  const outPath = '/tmp/classic-parity-probe.png';
  ...
  console.log('draw stats:', JSON.stringify(stats, null, 2));
  for (const y of [5, 60, 100, 150]) {
    const gold = await samplePlayfieldPng(GOLD, 160, y);
    const classic = await samplePlayfieldPng(outPath, 160, y);
    const delta = Math.max(
      Math.abs(gold[0]! - classic[0]!),
      Math.abs(gold[1]! - classic[1]!),
      Math.abs(gold[2]! - classic[2]!),
    );
    console.log(`y=${y} gold=${gold.join(',')} classic=${classic.join(',')} delta=${delta}`);
  }
}
