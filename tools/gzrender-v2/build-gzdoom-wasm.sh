#!/usr/bin/env bash
# GOLD ORACLE — GZDoom WASM parity reference. DO NOT merge with GZDoom (s) pure-WASM work.
#
# Output: doom-wad-lab/public/wasm/gzdoom/{gzdoom.js,gzdoom.wasm,*.pk3}
# Build dir: gzdoom-project/build-wasm (NEVER build-pure-wasm-s)
# Modular (s): tools/gzrender-v2/build-gzdoom-s-pure-wasm.sh → public/wasm/gzdoom-s/
#
# Build stripped GZDoom for Emscripten (browser oracle renderer).
#
# Output: doom-wad-lab/public/wasm/gzdoom/{gzdoom.js,gzdoom.wasm,*.pk3}
#
# Prerequisites:
#   - emscripten (emcc), ninja, working node (emcmake invokes node --version)
#   - Native GZDoom build in gzdoom-project/build (ImportExecutables + zipdir)
#   - zmusic submodule in gzdoom-project
#
# Usage: tools/gzrender-v2/build-gzdoom-wasm.sh
set -euo pipefail
...
OUT_DIR="$ROOT/public/wasm/gzdoom"
BUILD_DIR="$GZDOOM_ROOT/build-wasm"
USE_WASM64="${GZDOOM_WASM64:-1}"
if [[ "$USE_WASM64" == "0" ]]; then
  BUILD_DIR="$GZDOOM_ROOT/build-wasm32"
fi
LOG_DIR="$ROOT/artifacts/gzrender-v2/logs"
LOG_FILE="$LOG_DIR/build-gzdoom-wasm.log"
...
command -v emcc >/dev/null || die "emcc not found — install emscripten (brew install emscripten)"
...
ensure_node_for_emscripten() {
  ...
  log "node failed (often Homebrew node vs simdjson mismatch). Try: brew reinstall node"
...
resolve_emscripten_sdl2_include() {
  ...
  [[ -n "$sdl_h" ]] || die "SDL.h not in emscripten cache — run: emcc -sUSE_SDL=2 -o /dev/null /dev/null"
  dirname "$sdl_h"
}

# Emscripten ships an OpenAL → WebAudio implementation, but GZDoom normally dlopen()s libopenal
# (DYN_OPENAL), which cannot work in WASM. We compile the non-DYN static path against GZDoom's own
# bundled OpenAL Soft headers (al.h/alc.h/alext.h/efx.h — a self-consistent set that defines
# AL_APIENTRY etc.; Emscripten's minimal sysroot al.h omits those and breaks efx.h) and link the
# standard AL ABI symbols with -lopenal. al.h is included unprefixed in oalsound.h, so the include
# dir must be the dir that *contains* al.h.
...
ensure_native_import_executables() {
  ...
  [[ -f "$import" ]] || die "missing $import — build native GZDoom first (cmake + ninja in gzdoom-project/build)"
  printf '%s' "$import"
}
...
[[ -f "$PK3_SRC/gzdoom.pk3" ]] || die "gzdoom.pk3 not found — run build-gzdoom-pk3.sh"
...
if [[ "$USE_WASM64" == "0" ]]; then
  WASM_TAG="wasm32"
  MEM64_FLAG=""
  EMFLAGS="-fexceptions -isystem $OPENAL_INC"
  ...
  EMFLAGS="-sMEMORY64=1 -fexceptions -isystem $OPENAL_INC"
...
ZMusic_DIR="$GZDOOM_ROOT/zmusic"
LIBVPX_DIR="$GZDOOM_ROOT/libvpx-wasm"

if [[ ! -f "$BUILD_DIR/build.ninja" ]] || [[ "${FORCE_GZDOOM_WASM_RECONFIGURE:-0}" == "1" ]]; then
  log "cmake configure (Emscripten $WASM_TAG, single-threaded)..."
  rm -rf "$BUILD_DIR"
  mkdir -p "$BUILD_DIR"
  ...
  log "cmake configure: reusing $BUILD_DIR (delete dir to reconfigure)"
  cmake -S "$GZDOOM_ROOT" -B "$BUILD_DIR" \
    -DCMAKE_EXE_LINKER_FLAGS="$LINKFLAGS" \
    -DDYN_OPENAL=OFF \
    -DOPENAL_INCLUDE_DIR="$OPENAL_INC" \
    -DOPENAL_LIBRARY="-lopenal" \
...
log "ninja zdoom (output gzdoom.js + gzdoom.wasm)..."
...
log "build gzdoom-wasm-shaders.pk3 (deflate shader overlay for browser)..."
python3 "$SCRIPT_DIR/build-shader-overlay-pk3.py" "$OUT_DIR/gzdoom.pk3" "$OUT_DIR/gzdoom-wasm-shaders.pk3" >>"$LOG_FILE" 2>&1

log "SUCCESS: gzdoom.wasm $(wc -c <"$OUT_DIR/gzdoom.wasm" | tr -d ' ') bytes, gzdoom.js $(wc -c <"$OUT_DIR/gzdoom.js" | tr -d ' ') bytes"
